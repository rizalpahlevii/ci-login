...
class Auth extends CI_Controller{
    ...
    private function _login(){
        $role_user = $this->db->get('user_role')->result();
    ...
    public function registration(){
        ...
        $this->form_validation->set_rules('name', 'Name', 'required|trim|min_length[7]');
    ...
    private function _sendEmail($token, $type){
        ...
        $config = [
            ...
            'smtp_user' => 'loginprojectci@gmail.com',
            'smtp_pass' => '1234',
            'smtp_port' => 465,
            'mailtype' => 'html',
            'charset' => 'utf-8',
        ...
        $this->email->from('loginprojectci@gmail.com','Project CI');
        $this->email->to($this->input->post('email'));
        if($type == 'verivy'){
            $this->email->subject('Account Verivication');
            $this->email->message('Click this link to verify your account : <a href="'. base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Actiate</a>');
        }
        if($this->email->send()){
            return true;
        }else{
    ...
    public function verify(){
        ...
        $user = $this->db->get_where('user', ['user' => $email])->row_array();
        if($user){
